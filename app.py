...
if __name__ == "__main__":
    ...
    if (rank == 0): # в первой зоне
        ...
        for i in range(0, len(t) - 1):
            ...
            req = comm.isend(y[i+1][end], dest=1, tag=0)
    ...
    if (rank == P-1): # в последней зоне
        ...
        for i in range(0, len(t) - 1):
            ...
            req = comm.isend(y[i+1][zone*rank], dest=P-2, tag=0)
        ...
        for process in range(1, P-1):
            if (process == rank):
                ...
                for i in range(0, len(t) - 1):
                    ...
                    reqLeft = comm.isend(y[i+1][zone*rank], dest=rank-1, tag=0)
                    reqRight = comm.isend(y[i+1][end], dest=rank+1, tag=0)
...
# /usr/local/bin/mpicc
# mpiexec -n 4 python3 -m mpi4py mpi-feature.py